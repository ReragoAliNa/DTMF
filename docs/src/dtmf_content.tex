...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{../../fpga/waveform.png} % Placeholder, user needs to capture this
    \caption{ISim 仿真波形：展示了按键触发后的相位累加与正弦波合成过程}
\end{figure}

\section{附录：完整源代码}

\subsection{Python 核心算法}

\newpage
\lstinputlisting[caption={DTMF 核心算法实现 (src/core/dsp.py)}, language=Python]{../../src/core/dsp.py}

\newpage
\lstinputlisting[caption={自适应检测器 (src/ml/adaptive\_detector.py)}, language=Python]{../../src/ml/adaptive_detector.py}

\newpage
\lstinputlisting[caption={主程序入口 (src/main.py)}, language=Python]{../../src/main.py}

\subsection{Java Web 端核心逻辑}

\newpage
\lstinputlisting[caption={信号处理服务 (DtmfService.java)}, language=Java]{../../java-web/src/main/java/com/example/dtmf_web/service/DtmfService.java}

\newpage
\lstinputlisting[caption={RESTful 控制器 (DtmfController.java)}, language=Java]{../../java-web/src/main/java/com/example/dtmf_web/controller/DtmfController.java}

\subsection{FPGA VHDL 源码}

\newpage
\lstinputlisting[caption={顶层模块 (ax309\_top.vhd)}, language=VHDL]{../../fpga/ax309_top.vhd}

\newpage
\lstinputlisting[caption={DTMF 发生器 (dtmf\_generator.vhd)}, language=VHDL]{../../fpga/dtmf_generator.vhd}

\newpage
\lstinputlisting[caption={PWM 音频 (pwm\_audio.vhd)}, language=VHDL]{../../fpga/pwm_audio.vhd}
