...
\subsection{逻辑设计与仿真}
本设计在 Xilinx ISE 14.7 环境下进行工程管理（如图 \ref{fig:ise_project}），并通过 RTL 级仿真验证了设计逻辑的正确性。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{fpga/ise_project.png}
    \caption{Xilinx ISE 工程概览与模块层级}
    \label{fig:ise_project}
\end{figure}

图 \ref{fig:rtl_schematic} 展示了综合前的 RTL 原理图，清晰地反映了双路 DDS 相位累加器并行工作的电路结构。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{fpga/ise_rtl.png}
    \caption{RTL 原理图 (RTL Schematic)}
    \label{fig:rtl_schematic}
\end{figure}

为了验证核心算法，我们使用 GHDL 进行了仿真。图 \ref{fig:gtkwave} 展示了模拟按下数字键 \textbf{'1'} 时的波形状态。
\begin{enumerate}
...
\subsection{综合与时序分析}
完成设计输入后，进行逻辑综合 (Synthesize)。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{fpga/ise_synthesis.png}
    \caption{综合 (Synthesize) 过程报告}
    \label{fig:ise_synthesis}
\end{figure}

资源占用报告（图 \ref{fig:resource}）显示，设计仅占用了极少的 Slice LUTs 和 Block RAM，适合低成本 FPGA 实现。时序报告（图 \ref{fig:timing}）表明设计完全满足 50MHz 时钟约束，无建立/保持时间违例。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{fpga/ise_resource.png}
    \caption{FPGA 资源占用报告 (Device Utilization)}
    \label{fig:resource}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{fpga/ise_timing.png}
    \caption{时序分析报告 (Timing Summary)}
    \label{fig:timing}
\end{figure}

\subsection{物理实现与板级验证}
最后进行布局布线 (Implement Design) 并生成比特流文件。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{fpga/ise_implement.png}
    \caption{布局布线 (Implement Design) 成功状态}
    \label{fig:ise_implement}
\end{figure}

将程序下载至 AX309 开发板（图 \ref{fig:board}），连接示波器和扬声器进行测试。实测按下按键可听到准确的双音多频声音，且可通过板载 LED 观察按键编码状态。
