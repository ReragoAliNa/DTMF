...
本设计采用自顶向下的设计方法。在将代码下载到 FPGA 之前，首先对核心的 DDS 信号生成模块进行了严格的 RTL 级仿真。
图 \ref{fig:gtkwave} 展示了测试平台模拟按下数字键 \textbf{'1'} 时的 GHDL 仿真波形：
\begin{itemize}
    \item \textbf{绿色波形 (addr)}：代表相位累加器的输出。可以看到两种不同斜率的锯齿波，分别对应 697Hz (行频) 和 1209Hz (列频)。
    \item \textbf{红色波形 (data)}：代表查表后的正弦波输出。仿真结果证明了双音信号的频率和相位关系完全准确。
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{fpga/gtkwave_simulation.png}
    \caption{GHDL 仿真波形：按键 '1' 的双音信号生成过程验证}
    \label{fig:gtkwave}
\end{figure}

\subsection{工程构建与综合 (Project Implementation)}
逻辑验证通过后，在 Xilinx ISE 14.7 环境下进行完整的工程构建。
如图 \ref{fig:project_status} 所示，工程树中的 "Synthesize"（综合）、"Implement Design"（布局布线）以及 "Generate Programming File"（比特流生成）均显示为绿色对勾状态。这标志着设计已成功通过所有编译阶段，且满足时序约束，生成的 \texttt{.bit} 文件可用于物理下载。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{fpga/ise_resource.png}
    \caption{Xilinx ISE 工程全流程编译通过状态概览}
    \label{fig:project_status}
\end{figure}

\subsection{板级硬件测试 (Hardware Test)}
最后，将生成的比特流下载至 AX309 (Spartan-6) 开发板进行实测。测试环境如图 \ref{fig:board} 所示，通过示波器捕捉引脚输出，并驱动扬声器进行听觉验证。实验结果表明，按键触发灵敏，输出音调准确，达到了设计预期。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{fpga/fpga_board.png}
    \caption{FPGA 开发板实物连接与测试现场}
    \label{fig:board}
\end{figure}
