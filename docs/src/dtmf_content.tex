...
\subsection{硬件测试环境}
将比特流下载至 AX309 开发板，实际连接效果如图 \ref{fig:board} 所示。
...
\subsection{仿真验证}
为了验证 DDS 逻辑的正确性，我们使用 GHDL 进行了 RTL 级仿真，并使用 GTKWave 观察波形（如图 \ref{fig:gtkwave} 所示）。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{fpga/gtkwave_simulation.png}
    \caption{GHDL + GTKWave 仿真波形：DDS 核心逻辑验证}
    \label{fig:gtkwave}
\end{figure}

从波形图中可以清晰地观察到 DDS 技术的核心原理：
\begin{itemize}
    \item \textbf{相位累加 (锯齿波)}: 图中绿色的锯齿状波形（signal \texttt{addr}）代表相位累加器的高 8 位。它从 0 线性增加到 255 并循环溢出，模拟了对正弦波完整周期 ($0 \sim 2\pi$) 的扫描。由于行频和列频不同，上下两组锯齿波的周期（斜率）存在明显差异，上方波形更为密集，分别对应高频（列）和低频（行）信号。
    \item \textbf{波形查找 (正弦波)}: 曲线波形（signal \texttt{data}）是根据地址从查找表 (ROM) 中读取的正弦幅度值。每个锯齿波周期严格对应一个完整的正弦波周期，验证了相位到幅度的映射逻辑完全正确，即 $ \text{Output} = \sin(\text{Addr}) $。
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{实现 (Implement Design) 过程}
