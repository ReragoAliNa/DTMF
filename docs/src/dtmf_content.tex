...
\section{总体方案}
本设计采用"硬件生成—软件检测"的闭环验证架构，总体方案如图 \ref{fig:system_arch} 所示。其核心流程包括：
1. \textbf{信号源层}：由 FPGA 硬件或 Web 前端模拟生成标准的 DTMF 双音多频信号。
2. \textbf{传输中继层}：利用 Python 编写的异步串口网关，将 FPGA 输出的 PCM 原始采样流实时透传至后端。
3. \textbf{算法处理层}：基于 Java Spring Boot 构建的核心服务端，实现自适应 Goertzel 检测、噪声注入及性能评估。
4. \textbf{展现交互层}：通过 WebSocket 和 RESTful API 实现分析结果的实时可视化展示。

\begin{figure}[H]
    \centering
    \begin{tikzpicture}[node distance=1.5cm and 2.2cm, auto, thick]
        % 定义样式
        \tikzset{
            block/.style = {rectangle, draw, fill=blue!10, text width=8.5em, text centered, rounded corners, minimum height=3.5em, font=\small},
            line/.style = {draw, -latex'}
        }
        
        % 放置节点
        \node [block] (fpga) {FPGA 硬件生成};
        \node [block, right=of fpga] (bridge) {Python 串口网关};
        \node [block, right=of bridge] (backend) {Java 算法中枢};
        \node [block, below=of backend] (web) {Web 交互界面};
        \node [block, left=of web] (simu) {软件信号发生器};

        % 连接线
        \path [line] (fpga) -- node [above, font=\footnotesize] {UART} (bridge);
        \path [line] (bridge) -- node [above, font=\footnotesize] {HTTP} (backend);
        \path [line] (simu) -- node [pos=0.6, right, font=\footnotesize] {API} (backend);
        \path [line] (backend) -- node [right, font=\footnotesize] {WebSock} (web);
        \path [line, dashed] (web) -- node [above, font=\scriptsize] {控制参数} (simu);
    \end{tikzpicture}
    \caption{DTMF 软硬结合实验系统总体方案架构图}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{前端}：HTML5/CSS3/JS 实现响应式交互，集成 \texttt{Chart.js} 实时绘制时频双域图像。
    \item \textbf{硬件网关}：Python 编写的 \texttt{fpga\_uart\_bridge} 充当数据中继。由于目前主流 PC 已不再原生提供 RS-232 物理接口，本系统利用 AX309 的 UART-to-USB 串口进行原始音频采样流的高速透明传输。桥接程序实时读取串口 PCM 字节流，将其封装并以 HTTP POST 协议转发至 Java 后端，实现低延迟的闭环处理。
        \item \textbf{后端/算法核}：Java Spring Boot 实现自适应 Goertzel 算法，直接在接收到的原始波形上进行高精度能量检测与信噪比估计。
    \end{itemize}
\end{itemize}
...
具体频率组合遵循 CCITT 标准（见表 \ref{tab:dtmf_freq}）。该组合经过精心挑选，每一组频率之间均为互质关系，且二阶与三阶互调产物均避开了有效频点，具有极强的抗干扰能力。

\begin{table}[H]
    \centering
    \caption{DTMF 拨号音频频率映射表 (单位: Hz)}
    \begin{tabular}{|c|c|c|c|c|}
        \hline
        \textbf{频率 (Hz)} & \textbf{1209} & \textbf{1336} & \textbf{1477} & \textbf{1633} \\ \hline
        \textbf{697} & 1 & 2 & 3 & A \\ \hline
        \textbf{770} & 4 & 5 & 6 & B \\ \hline
        \textbf{852} & 7 & 8 & 9 & C \\ \hline
        \textbf{941} & * & 0 & \# & D \\ \hline
    \end{tabular}
    \label{tab:dtmf_freq}
\end{table}，这种双音频设计利用四个行频和四个列频的唯一组合，有效防止了单一频率信号（如人声谐波）导致的误触发。
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{theory/zplane.png}
        \caption{Goertzel 滤波器的 Z 平面零极点分布}
    \end{minipage}
    \hfill
    \begin{minipage}{0.45\textwidth}
        \centering
        \includegraphics[width=\textwidth]{theory/vector_walk.png}
        \caption{能量累积过程的复平面向量演变 (Vector Walk)}
    \end{minipage}
\end{figure}
...
\begin{enumerate}
    ...
    \item \textbf{前端界面 (HTML5/JS/JSP)}: 采用现代化的响应式设计，利用 \texttt{Chart.js} 库实时绘制信道的时域波形图与归一化的 Goertzel 能量分布直方图，并使用 Web Audio API 播放加噪音频。
...
\paragraph{单信号检测模式}
用户选择目标按键后，系统立即生成加噪 DTMF 信号并执行 Goertzel 算法检测。

\begin{figure}[H]
    \centering
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{PC/1.png}
        \caption{单信号检测 - 参数配置界面}
        \label{fig:exp_single1}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{PC/2.png}
        \caption{单信号检测 - 波形与能量分布}
        \label{fig:exp_single2}
    \end{minipage}
\end{figure}

\paragraph{算法对比模式}
点击 "Compare" 按钮后，系统同时运行标准 Goertzel (固定 200ms) 与自适应算法，并排显示两者的识别结果与性能指标。

\begin{figure}[H]
    \centering
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{PC/3.png}
        \caption{算法对比 - 性能指标对比}
        \label{fig:exp_compare1}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{PC/4.png}
        \caption{算法对比 - 能量分布对比}
        \label{fig:exp_compare2}
    \end{minipage}
\end{figure}
...
\paragraph{软件信号发生器模式}
用户通过 Web 界面的虚拟拨号盘输入按键，系统在后端合成 DTMF 信号并注入指定噪声，随后执行自适应检测。

\begin{figure}[H]
    \centering
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{PC/5.png}
        \caption{软件信号发生器 - 拨号界面}
        \label{fig:phone_sw1}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{PC/6.png}
        \caption{软件信号发生器 - 实时识别}
        \label{fig:phone_sw2}
    \end{minipage}
\end{figure}

\paragraph{FPGA 硬件信号源模式}
当 FPGA 开发板通过 USB-UART 连接至 PC 时，系统自动切换至硬件模式。物理按键触发 DDS 产生的真实 DTMF 波形通过串口传入 Java 后端，执行与软件模式相同的自适应检测流程。

\paragraph{全链路集成测试}
图 \ref{fig:fpga_key1} - \ref{fig:fpga_key4} 展示了 FPGA 硬件与 Web 系统的完整交互流程。分别按下开发板上的按键 1、2、3、4，观察 PC 端实时捕获的 PCM 波形、Goertzel 能量分布及识别结果：

\begin{figure}[H]
    \centering
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{PC/7.png}
        \caption{全链路测试 - 按键 1}
        \label{fig:fpga_key1}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{PC/8.png}
        \caption{全链路测试 - 按键 2}
        \label{fig:fpga_key2}
    \end{minipage}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{PC/9.png}
        \caption{全链路测试 - 按键 3}
        \label{fig:fpga_key3}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{PC/10.png}
        \caption{全链路测试 - 按键 4}
        \label{fig:fpga_key4}
    \end{minipage}
\end{figure}
...
\begin{itemize}
    \item \textbf{会话浏览与多选}: 自动扫描 \texttt{audio/} 目录，提供直观的会话列表，支持按文件夹批量加载与统计。
    \item \textbf{单文件细节透视}: 新增功能支持点击统计列表中的任意音频文件，实时复现该次按键生成的**原始时域波形**与**能量分布谱**。
    \item \textbf{批量统计}: 自动计算整个会话的成功率、平均识别时长及按键频次分布，通过直方图直观呈现。
\end{itemize}

\paragraph{检测配置 (Detection Settings) 详解}
分析模式允许用户在离线状态下调整关键算法参数，以评估不同配置对识别性能的影响：
\begin{enumerate}
    \item \textbf{自适应模式 (Adaptive Mode)}: 激活后，系统将自动在 40ms 至 1000ms 之间动态调整积分窗口。其核心逻辑是：先尝试 40ms 的超短窗口（Fast 模式），若 Goertzel 能量比值未达到置信区间，则阶梯递增窗口长度，直至在长达 1000ms 的 Deep 模式下捕捉到微弱的信号特征。
    \item \textbf{手动时长调节 (Signal Duration)}: 通过滑动条实现 40ms 至 1s 的精细化控制。
    \begin{itemize}
        \item \textbf{Fast (40ms)}: 用于验证系统的极限响应速度，通常仅在 SNR > 15dB 时表现可靠。
        \item \textbf{Standard (200ms)}: 系统默认基准，模仿传统 PSTN 终端的检测逻辑。
        \item \textbf{Deep (1000ms)}: 利用累积增益最大限度压制环境噪声，是分析极端恶劣信道记录的首选。
    \end{itemize}
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{PC/11.png}
    \caption{音频分析模式：会话列表、Detection Settings 面板与统计图表}
    \label{fig:analyze_main}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{PC/12.png}
        \caption{分析模式 - 批量统计结果}
        \label{fig:analyze_stats}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{PC/13.png}
        \caption{分析模式 - 单文件细节透视}
        \label{fig:analyze_detail}
    \end{minipage}
\end{figure}
...
\subsection{逻辑设计与仿真 (Design Verification)}
本设计采用自顶向下的设计方法。在将代码下载到 FPGA 之前，首先对顶层模块 \texttt{ax309\_top} 进行了严谨的仿真验证。
图 \ref{fig:gtkwave} 为基于 GHDL 引擎导出的 \texttt{gtkwave} 仿真谱图，展示了按下数字键 \textbf{'1'} 时的全链路响应过程：
\begin{enumerate}
    \item \textbf{输入与去抖 (0-20ms)}：观察信号左端可以发现约 20ms 的静默期，这证明了 \texttt{key\_debounce} 模块有效抑制了按键初期的机械震荡，只有在电平稳定 20ms 后才触发后级。
    \item \textbf{双音生成 (20ms 后)}：音频 PCM 数据 \texttt{pcm\_in} 开始输出复合信号。
    \item \textbf{串口外传}：上方的 \texttt{tx\_data} 指示了 UART 模块正同步将按键索引实时上传至 PC 环境。
\end{enumerate}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{fpga/simu/dataset.png}
    \caption{系统集成仿真波形 (\texttt{dataset\_tb})：展示按键去抖周期 (20ms) 与 PCM 信号生成的协同关系}
    \label{fig:gtkwave}
\end{figure}

\subsubsection{DDS 双音波形仿真分析}
为更清晰地观察 DTMF 信号发生器的输出特性，对 \texttt{dtmf\_generator} 模块进行了单独的波形仿真。图 \ref{fig:dtmf_wave} 展示了 DDS 核心产生的双音复合波形：

\begin{itemize}
    \item \textbf{波形包络}：可以观察到明显的拍频现象 (Beat Frequency)，这是行频 ($f_L$) 与列频 ($f_H$) 线性叠加后的正常特征。包络周期 $T_{beat} = \frac{1}{|f_H - f_L|}$。
    \item \textbf{相位累加器精度}：由于采用 32 位相位累加器，DDS 输出的频率分辨率高达 $\frac{50\text{MHz}}{2^{32}} \approx 0.012\text{Hz}$，远优于 ITU-T 规定的 $\pm 1.5\%$ 容差要求。
    \item \textbf{量化阶梯}：波形呈现轻微的阶梯状，这是 16 位 PCM 量化与 256 点正弦查找表共同作用的结果，属于正常的数字信号特征。
\end{itemize}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{fpga/simu/dtmf_wave.png}
    \caption{DDS 双音信号发生器输出波形：展示行频与列频叠加后的拍频特征}
    \label{fig:dtmf_wave}
\end{figure}

\subsection{工程构建与综合 (Project Implementation)}
逻辑验证通过后，在 Xilinx ISE 14.7 环境下进行完整的工程构建。
设计已成功通过所有编译阶段（包括 Synthesize, Implement Design, Generate Programming File），且满足时序约束，生成的 \texttt{.bit} 文件可用于物理下载。

\subsection{全链路集成测试 (Full-Chain Integration)}
最后，将生成的比特流下载至 AX309 (Spartan-6) 开发板，并配合 Python 串口网关与 Java Web 后端进行全链路实测。
测试场景如图 \ref{fig:board} 所示：
\begin{itemize}
    \item \textbf{FPGA 端}：按下按键，蜂鸣器鸣响，同时 UART 指示灯闪烁。
    \item \textbf{PC 端}：浏览器界面实时捕捉到来自硬件的波形流，自动计算 SNR 并在"自适应模式"下完成解码。
\end{itemize}
实验结果表明，从物理按键触发到 Web 界面显示识别结果的端到端延迟约为 150ms（含去抖与网络转发），频率识别误差 0\%，系统稳定性极佳。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{PC/8.png}
    \caption{软硬全链路集成验证：FPGA 生成信号通过 Python 网关透明上传至 Java 后端进行实时分析}
    \label{fig:board}
\end{figure}
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{fpga/fpga_board_1.png}
        \caption{AX309 开发板正面实物图}
    \end{minipage}
    \hfill
    \begin{minipage}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{fpga/fpga_board_4.png}
        \caption{按键触发时的 LED 指示状态}
    \end{minipage}
\end{figure}
...
\lstinputlisting[caption={src/core/dsp.py (部分截取)}, firstline=1, lastline=58]{../../src/core/dsp.py}

\subsection{Java 后端核心逻辑 (Spring Service)}
\lstinputlisting[language=Java, caption={DtmfService.java (Goertzel 核心算法)}, firstline=344, lastline=407]{../../java-web/src/main/java/com/example/dtmf_web/service/DtmfService.java}

\lstinputlisting[language=Java, caption={DtmfService.java (自适应渐进探测逻辑)}, firstline=409, lastline=481]{../../java-web/src/main/java/com/example/dtmf_web/service/DtmfService.java}

\subsection{FPGA 信号发生器逻辑 (VHDL)}
\lstinputlisting[caption={fpga/dtmf\_generator.vhd (核心逻辑)}, firstline=40, lastline=108]{../../fpga/dtmf_generator.vhd}
